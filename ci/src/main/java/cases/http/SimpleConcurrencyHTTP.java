...
public class SimpleConcurrencyHTTP implements E2ECase {
    ...
    public SimpleConcurrencyHTTP(MxBuilder builder, mxgateHelper mxgateHelper) {
    ...
    public void execute() throws AssertionError, IOException, SQLException {
        ...
        // start mxgate
        try {
            mxgateHelper.startHttp(dbHelper.getDBName(), schema, tableName1, "unix-ms");
        } catch (Exception e) {
            throw new RuntimeException("failed to start mxgate", e);
        }
        PrintColor.BluePrint("Succeed to starting mxgate");
        ...
        this.builder.connect(httpURL, grpcURL, schema, tableName1, new ConnectionListener() {
            @Override
        ...
        this.builder.connect(httpURL, grpcURL, schema, tableName2, new ConnectionListener() {
            @Override
            public void onFailure(String failureMsg) {
